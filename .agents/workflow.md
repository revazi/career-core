...
The Phase 9 `npm-cli-packages.yml` workflow remains manual preparation-only. The existing `npm-publish.yml` records the completed exact `v0.1.0` publication path and must not be repurposed or dispatched for proposed `0.1.1` during intermediate matrix PRs. A protected `v0.1.1` nine-package path may be prepared only in the final sub-gate after all eight native execution jobs are green. It must preserve `npm-production`, pinned tooling/actions, bootstrap/OIDC isolation, exact npm provenance/integrity checks, all eight native packages before the launcher, and final no-secret native public acceptance. It publishes no crate, signature, notarization, GitHub Release, or custom asset. Do not dispatch publication during implementation.
