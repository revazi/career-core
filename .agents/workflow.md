...
The Phase 9 `npm-cli-packages.yml` workflow remains manual preparation-only. Historical `npm-publish.yml` and `npm-publish-v0.1.1.yml` record immutable completed release paths and are not templates for future versions. Stable `npm-release.yml` derives the version from an annotated stable-SemVer tag, requires exact tag/main/reviewed-SHA and Cargo/npm lockstep, uses OIDC only through approved `npm-production`, publishes all eight native packages before the launcher, verifies exact npm provenance/integrity, and runs no-secret public acceptance on all eight targets. It publishes no crate, signature, notarization, GitHub Release, or custom asset. Implementation sessions must not dispatch it, authenticate/query npm, create tags/releases, or use credentials.
