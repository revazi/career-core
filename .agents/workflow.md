...
The Phase 9 `npm-cli-packages.yml` workflow remains manual preparation-only. Stable `npm-release.yml` derives the version from an annotated stable-SemVer tag, requires exact tag/main/reviewed-SHA and Cargo/npm lockstep, uses OIDC only through approved `npm-production`, publishes all six native packages before the launcher, verifies exact npm provenance/integrity, and runs no-secret public acceptance on all six targets. It publishes no crate, signature, notarization, GitHub Release, or custom asset. Implementation sessions must not dispatch it, authenticate/query npm, create tags/releases, or use credentials.
