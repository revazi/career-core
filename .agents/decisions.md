...
## D-023 — The macOS/Linux matrix requires exact native evidence

The reviewed ordered target catalog contains Darwin ARM64/x64, GNU Linux x64/ARM64, and musl Linux x64/ARM64. Launcher/native package identities remain exact and lockstep; internal launcher, native metadata, and provenance use v2 so libc family, binary format/architecture, executable filename, archive mode, and Unix file invariants are explicit.

A catalog entry, private template, synthetic header, cross-compiled artifact, emulator result, or skipped CI job is not platform support. Every final packaged binary must execute on the exact native OS and architecture in CI. Linux must be positively classified as glibc or musl; missing glibc evidence never implies musl. GNU packages require a supported glibc 2.35 floor and reject imported symbols above that floor.
...
Stable releases use one `npm-release.yml`. It accepts only an exact reviewed SHA while deriving stable `X.Y.Z` from annotated `vX.Y.Z`; the tag, exact fetched `origin/main`, root/workspace Cargo metadata, `Cargo.lock`, launcher, active native templates, optional dependencies, candidate filenames, provenance, publication plan, and public acceptance must all agree. Prerelease/build tags and version constants in the stable path are rejected.

Active npm trusted publishers bind once to repository `revazi/career-core`, workflow `npm-release.yml`, environment `npm-production`, and publish permission. Routine publication is OIDC-only and has no bootstrap input, npm secret, token npmrc, or fallback. Repository tag rules protect `v*`; the environment uses required maintainer approval and one `v*` deployment-tag rule. Native-before-launcher order, exact idempotent recovery, bounded subprocess/readiness behavior, registry integrity/SLSA provenance, and native public acceptance remain mandatory.

## D-028 — Distribution supports macOS and Linux only

Native Windows is unsupported in Career Core source and distribution. The root `career-core` library rejects `target_os = "windows"` at compile time, so the CLI and every adapter that depends on the root crate inherit the native Windows prohibition. This does not reject macOS, Linux, or existing Apple iOS/Swift targets. The reviewed catalog, launcher optional dependencies, private templates, preparation/release workflows, inspection/candidate/public-package code, and acceptance tests contain only six target classes: Darwin ARM64/x64, GNU Linux ARM64/x64, and musl Linux ARM64/x64. The launcher declares npm `os` support for `darwin` and `linux`; no native-Windows target, binary format, executable route, or package exists in the current tree.

Windows users may build and run Career Core and pi-career through WSL. In WSL, Rust and Node target Linux, and Node reports `process.platform === "linux"`, so the normal Linux architecture and positive glibc/musl policy applies. WSL support does not authorize a native Windows implementation. Routine publication contains six native packages followed by the launcher (seven total) with OIDC/provenance/idempotence policy unchanged.

## D-029 — Native-Windows removal advances the package coordinate to `0.2.0`

Removing an operating system from source and distribution is a breaking public platform change. Under pre-1.0 SemVer, the first six-target macOS/Linux-only release therefore advances Cargo, CLI, result `core_version`, launcher, and all six native package coordinates together to exact `0.2.0`. Public v1 operation/schema semantics, algorithms, limits, ordering, evidence, warnings, uncertainty, and assisted authority remain unchanged. The stable tag-derived OIDC workflow must prove all six final public packages before any consumer, including pi-career, may migrate from its prior exact coordinate.
