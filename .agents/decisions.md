...
For `v0.1.0`, the sole publish workflow is manual and protected by `npm-production`. Explicit `bootstrap=true` uses one temporary granular token for first publication or exact interrupted recovery. Default `bootstrap=false` forbids tokens and uses package-level GitHub OIDC trusted publishing. Neither mode falls back. Exact registry integrity and SLSA provenance are mandatory, conflicts fail, and completion requires no-secret public acquisition/parity on both hosts. The bootstrap secret/token is deleted/revoked immediately, then all package trusted publishers are configured and verified interactively with account 2FA. Only afterward does the parent maintainer create the annotated `v0.1.0` GitHub Release with dated notes and no custom assets. The transitional `pi-career` artifact path remains until the external consumer migration gate passes.

## D-023 — Proposed `0.1.1` matrix is cataloged before it is claimed

The next immutable npm version is patch `0.1.1`, preserving every public v1 operation/schema semantic while advancing exact Core version metadata. Its one reviewed ordered target catalog contains Darwin ARM64/x64, GNU Linux x64/ARM64, musl Linux x64/ARM64, and MSVC Windows x64/ARM64. Launcher/native package identities remain exact and lockstep; internal launcher, native metadata, and provenance move to v2 so libc family, binary format/architecture, executable filename, archive mode, and target-specific Unix/Windows file invariants are explicit.

A catalog entry, private template, synthetic header, cross-compiled artifact, emulator result, or skipped CI job is not platform support. Every final packaged binary must execute on the exact native OS and architecture in CI. Linux must be positively classified as glibc or musl; missing glibc evidence never implies musl. GNU packages require a supported glibc 2.35 floor and reject imported symbols above that floor. Windows uses `career.exe`, regular non-symlink file checks, and exact PE32+ machine verification instead of a Unix executable-mode claim.

The extension proceeds through separately approved target groups and ends with an exact eight-native-plus-launcher candidate. Public `0.1.0` packages, workflow bytes, and evidence remain immutable. Final `npm-publish-v0.1.1.yml` is a separate protected path with launcher-last publication and all-eight public acceptance. Implementation sessions may prepare and review that path but may not dispatch it, authenticate/query npm, create tags/releases, use credentials, or edit pi-career.
...
Each package executes every reviewed operation through both the extracted native executable and launcher. Bounded inspection requires the observed target-specific static form: x86-64 is ELF `DYN` static PIE with `NOW PIE` flags and a relocation-only dynamic section, while AArch64 is ELF `EXEC` with no dynamic section. Both require no interpreter, shared-library import, or GLIBC symbol claim, plus exact source/catalog/version/mode/size/SHA-256 binding. The digest-pinned container is build evidence, not a distributed runtime requirement. Evidence remains external log data and is neither uploaded nor published; final release source must reprove both musl targets.

## D-026 — Windows evidence is native PE policy without Unix-mode claims

The Windows sub-gate uses exact `windows-2025` x64 and `windows-11-arm` ARM64 hosted runners. Runner, process OS, Python, Node 22.19.0, and rustc/Cargo 1.97.1 host architecture must all agree with the reviewed MSVC target; cross-compilation, x64 emulation as ARM64 evidence, and unavailable-runner skips are not accepted.

A dedicated standard-library-only Windows staging path preserves the shared catalog, private templates, provenance v2, offline npm install, and all-operation launcher/native parity while using `career.exe` and archive mode `0644`. Bounded pure-Python PE inspection requires PE32+ executable and exact `0x8664` or `0xaa64` machine, parses the import directory with section/RVA bounds, and accepts only reviewed Windows system DLL basenames. Windows requires a regular non-symlink file but records no POSIX runtime mode. Evidence stays outside the checkout and is neither uploaded nor published; final release source must reprove both Windows targets.

## D-027 — Routine npm releases use one tag-derived OIDC-only workflow

After completed `v0.1.1` publication established the package identities, later releases use one stable `npm-release.yml`. It accepts only an exact reviewed SHA while deriving stable `X.Y.Z` from annotated `vX.Y.Z`; the tag, exact fetched `origin/main`, root/workspace Cargo metadata, `Cargo.lock`, launcher, active native templates, optional dependencies, candidate filenames, provenance, publication plan, and public acceptance must all agree. Prerelease/build tags and version constants in the stable path are rejected.

Active npm trusted publishers bind once to repository `revazi/career-core`, workflow `npm-release.yml`, environment `npm-production`, and publish permission. Routine publication is OIDC-only and has no bootstrap input, npm secret, token npmrc, or fallback. Repository tag rules protect `v*`; the environment uses required maintainer approval and one `v*` deployment-tag rule. Native-before-launcher order, exact idempotent recovery, bounded subprocess/readiness behavior, registry integrity/SLSA provenance, and active native public acceptance remain mandatory. Version-specific `v0.1.0`/`v0.1.1` workflows remain immutable historical evidence.

## D-028 — Active distribution supports macOS and Linux only (supersedes native Windows portions of D-023, D-026, and D-027)

Native Windows support is removed from active Career Core source and future distribution. The root `career-core` library rejects `target_os = "windows"` at compile time, so the CLI and every adapter that depends on the root crate inherit the native Windows prohibition. This does not reject macOS, Linux, or existing Apple iOS/Swift targets. The reviewed catalog, launcher optional dependencies, private templates, preparation/release workflows, inspection/candidate/public-package code, and acceptance tests contain only six target classes: Darwin ARM64/x64, GNU Linux ARM64/x64, and musl Linux ARM64/x64. The launcher declares npm `os` support for `darwin` and `linux`; native `win32`, PE, and `career.exe` paths fail closed because no active target exists.

Windows users may build and run Career Core and pi-career through WSL. In WSL, Rust and Node target Linux, and Node reports `process.platform === "linux"`, so the normal Linux architecture and positive glibc/musl policy applies. WSL support does not authorize a native Windows implementation.

Protected `v0.1.1` run `31346152236`, D-023's original eight-target proposal, and D-026 remain accurate historical evidence that the published `0.1.1` release included native Windows x64/ARM64 artifacts. Those bytes and version-specific workflows are immutable and must not be rewritten to erase history. They are superseded for active/future support. Routine publication now contains six native packages followed by the launcher (seven total) with OIDC/provenance/idempotence policy unchanged.
