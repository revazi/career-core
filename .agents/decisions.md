...
After completed `v0.1.1` publication establishes all nine package names, later releases use one stable `npm-release.yml`. It accepts only an exact reviewed SHA while deriving stable `X.Y.Z` from annotated `vX.Y.Z`; the tag, exact fetched `origin/main`, root/workspace Cargo metadata, `Cargo.lock`, launcher, eight native templates, optional dependencies, candidate filenames, provenance, publication plan, and public acceptance must all agree. Prerelease/build tags and version constants in the stable path are rejected.

All nine npm trusted publishers bind once to repository `revazi/career-core`, workflow `npm-release.yml`, environment `npm-production`, and publish permission. Routine publication is OIDC-only and has no bootstrap input, npm secret, token npmrc, or fallback. Repository tag rules protect `v*`; the environment uses required maintainer approval and one `v*` deployment-tag rule. Native-before-launcher order, exact idempotent recovery, bounded subprocess/readiness behavior, registry integrity/SLSA provenance, and all-eight native public acceptance remain mandatory. Version-specific `v0.1.0`/`v0.1.1` workflows remain immutable historical evidence.
