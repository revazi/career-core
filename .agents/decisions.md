...
## D-023 — `0.1.1` broadens the macOS/Linux matrix under exact native evidence

Patch `0.1.1` preserves every public v1 operation/schema semantic while advancing exact Core version metadata. Its reviewed ordered target catalog contains Darwin ARM64/x64, GNU Linux x64/ARM64, and musl Linux x64/ARM64. Launcher/native package identities remain exact and lockstep; internal launcher, native metadata, and provenance use v2 so libc family, binary format/architecture, executable filename, archive mode, and Unix file invariants are explicit.
