...
Checked-in templates remain private. The proposed `0.1.1` external candidate boundary requires clean annotated `v0.1.1` at exact fetched `origin/main`, reviewed SHA, exact Node/npm and Rust toolchains, approved native runners, strict tar/file/mode/source-byte allowlists, ordered eight-native-before-launcher assembly, and registry-free adversarial verification. The protected `v0.1.1` workflow remains deferred until every native sub-gate passes. npm registry provenance, package-contained SHA-256 consistency, and the absent independent native-binary signature remain distinct.
