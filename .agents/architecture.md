...
The root `career-core` library rejects `target_os = "windows"` at compile time, so every inward-dependent CLI or adapter build inherits the native Windows prohibition. WSL targets Linux and remains the Windows-host route; macOS, Linux, and existing Apple iOS/Swift targets are unaffected.

The Phase 9 `@revazi/career` npm launcher is a distribution adapter around the unchanged native CLI and the only npm consumer surface. Platform packages are internal exact-version optional-dependency implementation details. Active source and future releases support six Darwin/Linux target classes through one exact ordered catalog for package identities, Rust/Node/libc mappings, formats/architectures, size ceilings, provenance requirements, and Unix file invariants. The launcher declares npm `os` values `darwin` and `linux`, accepts only reviewed catalog bytes, resolves only one exact package, requires positive glibc or musl evidence rather than treating non-glibc as musl, and validates Mach-O or ELF architecture before direct execution. Every native executable is a regular non-symlink exact-mode `career` file. Native Windows, PE, and `career.exe` paths are absent. Windows hosts use Linux through WSL, where Node reports `process.platform === "linux"`.
...
Checked-in templates remain private. The Darwin/GNU evidence boundary executes offline extracted private packages on exact native ARM64/x64 macOS and GNU Linux hosts, then writes bounded external `career.npm_native_inspection.v1` evidence for source/catalog/host/version/header/linkage/import/interpreter/GLIBC-symbol/mode/size/SHA-256 checks. The musl boundary uses a digest-pinned Node 22.19.0 Alpine 3.22 image on architecture-matched native x64/ARM64 hosted runners and additionally proves exact musl 1.2.5 plus target-specific static linkage: x64 ELF `DYN` static PIE with `NOW PIE` flags, AArch64 ELF `EXEC`, and neither with an interpreter or shared-library imports. The preparation-only workflow uploads and publishes nothing. The stable external candidate boundary requires a clean annotated stable-SemVer `vX.Y.Z` at exact fetched `origin/main`, reviewed SHA, tag-aligned lockstep Cargo/npm versions, exact Node/npm and Rust toolchains, approved native runners, strict tar/file/mode/source-byte allowlists, ordered six-native-before-launcher assembly, and registry-free adversarial verification. Protected OIDC-only `npm-release.yml` repeats native execution/inspection from final source, publishes through approved `npm-production`, and requires no-secret public acceptance on all six targets. Version-specific `v0.1.0`/`v0.1.1` workflows are immutable historical evidence; the old `0.1.1` Windows artifacts remain history rather than active support. npm registry provenance, package-contained SHA-256 consistency, and the absent independent native-binary signature remain distinct.
