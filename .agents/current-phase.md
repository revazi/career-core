...
**Phase 9 maintenance — exact `@revazi/career@0.1.1` publication is complete on all eight native target classes. The active bounded follow-up replaces release-specific publication with one stable OIDC-only workflow for later annotated stable-SemVer tags. Phases 0–8 and historical `v0.1.0`/`v0.1.1` release evidence remain immutable.**
...
The fifth sub-gate completed through historical `npm-publish-v0.1.1.yml`: eight exact native packages in catalog order, launcher ninth, exact integrity/SLSA provenance, and no-secret public acceptance on all eight native targets. The active follow-up adds stable `npm-release.yml`, derives `X.Y.Z` from annotated `vX.Y.Z`, verifies lockstep Cargo/npm metadata, dynamically names all candidate/public artifacts, and permits OIDC only. All nine package names must already exist with trusted publishers bound once to the stable workflow. `npm-production` uses required approval plus a protected `v*` tag policy rather than one rule per release. Implementation work must not dispatch publication, authenticate/query npm, create tags/releases, use credentials, or edit pi-career.

Local stable-workflow verification passed generic current/future stable-SemVer source fixtures, prerelease/lightweight/moved/dirty/non-main/version failures, dynamic nine-package candidate filenames and provenance, OIDC all-package/partial/idempotent/conflict/absent-name/transient/auth/output-bound fake-registry behavior, visible bounded readiness, native-before-launcher order, historical workflow hashes, YAML/shell/Python syntax, launcher/private-package/installed-CLI tests, the complete Rust/CLI/managed-adapter/runtime-artifact ladder, and Fallow changed/all/security with zero correctness or security findings. No publication workflow, npm query/authentication, tag/release, credential, or pi-career edit occurred.
