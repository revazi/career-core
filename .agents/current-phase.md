...
The active third sub-gate adds exact native static musl x64/ARM64 packaged-binary execution and linkage evidence in pinned Alpine 3.22/musl 1.2.5 userlands on architecture-matched hosted Linux runners. The manual workflow must positively prove host/container architecture, musl runtime, x64 ELF `DYN` static PIE/AArch64 ELF `EXEC`, no interpreter or shared-library imports, exact package/launcher parity, and bounded source/catalog/version/mode/size/SHA-256 evidence without emulation, upload, or publication. Later sub-gates remain strictly ordered: Windows native evidence; then the exact nine-package protected candidate and final documentation. Do not begin a later sub-gate without explicit maintainer approval. The protected `v0.1.1` workflow remains deferred until the final publication-candidate sub-gate; the existing `v0.1.0` workflow and public bytes remain historical and immutable.
