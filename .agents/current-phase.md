...
Native Windows package templates, staging/process/test scripts, launcher binary-format/executable routes, preparation/release jobs, shared candidate/public-package handling, version-specific workflow, and checked-in historical evidence records are removed. The root library has a compile-time native Windows rejection inherited by the CLI and adapters. WSL remains supported as Linux because Rust and Node target Linux there; unknown or native Windows hosts fail closed.

The extension retains one user-facing npm surface, `@revazi/career`, and six exact internal optional packages. Catalog entries, synthetic fixtures, and cross-compilation remain policy data, not release evidence; every version must prove final public bytes natively on Darwin ARM64/x64, GNU Linux x64/ARM64, and musl Linux x64/ARM64.
