...
The active second sub-gate adds exact native Darwin x64 and GNU Linux ARM64 packaged-binary evidence while reproving Darwin ARM64 and GNU Linux x64. The manual workflow is preparation-only and must pass all four exact native jobs; implementation alone is not evidence. Later sub-gates remain strictly ordered: musl native evidence; Windows native evidence; then the exact nine-package protected candidate and final documentation. Do not begin a later sub-gate without explicit maintainer approval. The protected `v0.1.1` workflow remains deferred until the final publication-candidate sub-gate; the existing `v0.1.0` workflow and public bytes remain historical and immutable.
...
Second-sub-gate implementation-head run `31321990554` at `1949638` passed all four exact native private packaged-binary jobs: `macos-14` ARM64, `macos-15-intel` x64, `ubuntu-22.04` x64/glibc 2.35, and `ubuntu-24.04-arm` with native Ubuntu 22.04 ARM64/glibc 2.35. Every job used rustc/Cargo 1.97.1 and Node 22.19.0, passed offline extracted-package all-operation parity, and emitted bounded source/catalog/host/version/format/linkage/import/interpreter/mode/size/SHA-256 evidence without upload or publication. Both GNU binaries imported no GLIBC symbol newer than 2.34, within the proposed 2.35 floor. This intermediate run proves the four runner/package paths but is not a public support claim; the exact final PR head must pass before merge, and final release source must reprove all eight targets.
