...
**Phase 9 maintenance — active source and future `@revazi/career` distribution support six macOS/Linux native target classes plus launcher. The root crate rejects `target_os = "windows"`, so Career Core, the CLI, and adapters cannot build natively on Windows; Windows users build and run through WSL as Linux. Existing Apple iOS/Swift targets remain unchanged. Historical `v0.1.0`/eight-target `v0.1.1` release evidence remains immutable.**

## Active macOS/Linux distribution maintenance

The launcher declares npm `os` support for `darwin` and `linux`, and its ordered catalog contains Darwin ARM64/x64, GNU Linux x64/ARM64, and musl Linux x64/ARM64 only. Future OIDC candidates and publication contain six native packages followed by the launcher, with fail-closed platform/architecture/libc selection, direct execution, integrity/provenance validation, no install scripts/network, and all deterministic Core/JSON/Swift contracts unchanged.

Native Windows package templates, staging/process/test scripts, launcher PE/`career.exe` routes, active preparation/release jobs, and shared active candidate/public-package handling are removed. The root library has a compile-time native Windows rejection inherited by the CLI and adapters. WSL remains supported as Linux because Rust and Node target Linux there; unknown or native `win32` hosts fail closed.

Protected run `31346152236` remains accurate historical evidence that public `0.1.1` included two Windows packages and passed eight native acceptance jobs. Version-specific workflows remain immutable and the old artifacts cannot be withdrawn by changing source. They no longer define active or future platform support.

## Phase 9 extension historical scope

The extension retains one user-facing npm surface, `@revazi/career`, and exact internal optional packages. Public patch `0.1.1` supports Darwin ARM64/x64, GNU Linux x64/ARM64, musl Linux x64/ARM64, and MSVC Windows x64/ARM64 through one reviewed ordered target catalog. Protected bootstrap/public-acceptance run `31346152236` passed all eight native target jobs and published the launcher ninth. Catalog entries and cross-compilation remain policy data, not release evidence; every later version must reprove final public bytes natively.
...
The fifth sub-gate completed through historical `npm-publish-v0.1.1.yml`: eight exact native packages in catalog order, launcher ninth, exact integrity/SLSA provenance, and no-secret public acceptance on all eight native targets. The active follow-up uses stable `npm-release.yml`, derives `X.Y.Z` from annotated `vX.Y.Z`, verifies lockstep Cargo/npm metadata, dynamically names all candidate/public artifacts, and permits OIDC only. All seven active package names must already exist with trusted publishers bound once to the stable workflow; the two historical Windows package names are not part of the active release set. `npm-production` uses required approval plus a protected `v*` tag policy rather than one rule per release. Implementation work must not dispatch publication, authenticate/query npm, create tags/releases, use credentials, or edit pi-career.

Current local stable-workflow verification passes generic current/future stable-SemVer source fixtures, prerelease/lightweight/moved/dirty/non-main/version failures, dynamic seven-package candidate filenames and provenance, OIDC all-package/partial/idempotent/conflict/absent-name/transient/auth/output-bound fake-registry behavior, visible bounded readiness, native-before-launcher order, historical workflow hashes, YAML/shell/Python syntax, launcher/private-package/installed-CLI tests, the complete Rust/CLI/managed-adapter/runtime-artifact ladder, and Fallow changed/all/security with zero correctness or security findings. No publication workflow, npm query/authentication, tag/release, credential, or pi-career edit occurred.
