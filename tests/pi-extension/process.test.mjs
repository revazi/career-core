...
} from "../../extensions/career-core/process.ts";
...
before(async () => {
  ...
  executable = path.join(temporaryDirectory, "fake-career");
  shellMarker = `career-core-shell-marker-${randomUUID()}`;
...
test("uses direct argv and exact stdin without shell interpolation or payload files", async () => {
  ...
  const source = await readFile(
    fileURLToPath(new URL("../../extensions/career-core/process.ts", import.meta.url)),
    "utf8",
  );
