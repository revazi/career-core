...
Verify the exact clean, annotated, unmoved v0.1.0 publication source. The tag
...
[[ "$expected_ref" == "refs/tags/v0.1.0" ]] || fail "expected ref must be exactly refs/tags/v0.1.0"
...
git show-ref --verify --quiet refs/tags/v0.1.0 || fail "exact v0.1.0 tag is missing"
tag_type="$(git cat-file -t refs/tags/v0.1.0 2>/dev/null)" || fail "v0.1.0 tag object is unreadable"
[[ "$tag_type" == "tag" ]] || fail "v0.1.0 must be an annotated tag, not a lightweight tag"
tag_commit="$(git rev-parse 'refs/tags/v0.1.0^{commit}')"
[[ "$tag_commit" == "$head_sha" ]] || fail "v0.1.0 does not resolve to the reviewed HEAD commit"
...
if cargo_versions.get("package") != version:
    raise SystemExit("career-core package version is not exactly 0.1.0")
if cargo_versions.get("workspace.package") != version:
    raise SystemExit("workspace package version is not exactly 0.1.0")

launcher = load_json(root / "npm/career/package.json")
platforms = [
    load_json(root / "npm/platforms/darwin-arm64/package.json"),
    load_json(root / "npm/platforms/linux-x64-gnu/package.json"),
]
if launcher.get("name") != launcher_name or launcher.get("version") != version:
    raise SystemExit("launcher name/version is not the approved initial release")
...
for manifest, expected_name in zip(platforms, native_names):
    if manifest.get("name") != expected_name or manifest.get("version") != version:
        raise SystemExit("native template name/version is not the approved initial release")
    if manifest.get("private") is not True:
        raise SystemExit("source native templates must retain private: true")
PY

printf 'Verified exact clean annotated publication source v0.1.0 at %s on origin/main.\n' "$head_sha"
