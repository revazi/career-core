...
The native Pi extension invokes only an installed `career` executable with a direct argv array. It accepts a bounded absolute `CAREER_CLI_PATH` override, sends document JSON through stdin, enforces operation-specific input limits, timeout/cancellation cleanup, separate stream ceilings, exactly-one-object framing, and a complete-result context ceiling. It creates no career payload/result file, invokes no Cargo/shell/network/model/provider behavior, and exposes raw CLI detail only when it is a strictly bounded allowlisted `career.error.v1`; all other process errors are payload-free.

Pi itself may persist tool arguments and results in session JSONL. That host behavior is outside Core's no-persistence property. Private use requires an explicit persistence decision and should use a new `pi --no-session` run when transient behavior is desired. Neither ephemeral mode nor package/CLI uninstall is a secure-erasure guarantee.

Hosts that send resumes to an external model must obtain separate user consent, keep API keys outside core contracts and logs, bound provider context, isolate failures, and label accepted values as assisted. Consent to local Pi session handling is not provider consent.
